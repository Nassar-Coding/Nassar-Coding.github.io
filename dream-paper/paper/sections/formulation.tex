...
observations. Section~\ref{sec:data} details sources, exclusions, harmonization, and
...
Endogenous features use observations through day $t$ only, and calendar
features of day $t{+}1$ are deterministic. Realized target-day weather
is used as an oracle proxy for an accurate day-ahead forecast
(assumption A3): the resulting weather gain is an upper-information
experiment rather than an evaluation of an operational forecast
provider, bounded below by a lagged-weather variant.
...
an instrument for measuring the forecast-to-decision mapping under a
known objective, every parameter exposed as configuration, every step
auditable. It models no city's actual operations; any difference in
simulated outcome is attributable, by construction, to the forecasts
and the policy rule.
...
trend; Section~\ref{sec:robust} reports a trend diagnostic and the inference framing
follows it.
...
index). Degenerate point forecasts produce piecewise-flat marginal
values under this equal-weight coverage objective, so more than one
active family can attain the maximum marginal value at a sequential
allocation step; at such steps the realized allocation, and hence its
loss, depends on that key. We treat the key as a first-class object and
report the measured tie shares in Section~\ref{sec:decision}. A
...
losses, paired daily differentials with block-bootstrap intervals, and
reduction versus the uniform floor), asking when
